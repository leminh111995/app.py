...
with tab3:
    ...
    # Thử API Khối Ngoại — KHÔNG block UI nếu lỗi
    df_for = None
    net_v  = 0.0
    try:
        df_for = get_foreign(ticker, FOREIGN_DAYS)
    ...
    if valid(df_for):
        ...
        buy_v  = to_billion(row_to_buy(last_f))
        sell_v = to_billion(row_to_sell(last_f))
        net_v  = to_billion(row_to_net(last_f, buy_v, sell_v))
        ...
        net_v = 0.0
        st.warning(
            "⚠️ **API Khối Ngoại chưa hỗ trợ vnstock 4.x** — "
            "Đang dùng mô hình Ước Lượng từ Vol thay thế."
        )
    ...
    if st.button("🔥 KÍCH HOẠT RADAR TRUY QUÉT 3 TẦNG (REAL-TIME)"):
        ...
        for i, t in enumerate(scan_list):
            ...
            try:
                ...
                # Tổ Chức Gom: dùng PV Trend thay API (không gọi thêm API)
                smart = df_s['pv_trend'].tail(5).sum() >= 2
