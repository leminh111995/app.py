# ==============================================================================
# HỆ THỐNG QUẢN TRỊ ĐẦU TƯ ĐỊNH LƯỢNG - QUANT SYSTEM V10.6 (THE FINAL EXECUTION)
# ==============================================================================
# CHỦ SỞ HỮU: MINH
# CAM KẾT V10.6:
# 1. FIX TRIỆT ĐỂ LỖI NAMEERROR: Xóa đuôi version ở hàm để đồng bộ 100%.
# 2. KHÔNG VIẾT TẮT, KHÔNG NÉN MÃ NGUỒN, TƯỜNG MINH TỪNG DÒNG LỆNH.
# 3. GIỮ NGUYÊN ĐỘ CHI TIẾT CỦA ADVISOR LOGIC & MASTER CHART.
# ==============================================================================
...
def xac_thuc_quyen_truy_cap_cua_minh():
    ...
    Fix lỗi KeyError: Không dùng 'del' với widget key, thay bằng cách làm rỗng chuỗi.
    """
    ...
    if "trang_thai_dang_nhap_thanh_cong" not in st.session_state:
        st.markdown("### 🔐 Quant System V10.6 - Cổng Bảo Mật")
...
if xac_thuc_quyen_truy_cap_cua_minh():
    ...
    st.set_page_config(
        page_title="Quant System V10.6 Execution", 
        layout="wide",
        initial_sidebar_state="expanded"
    )
    
    # Tiêu đề giao diện chính
    st.title("🛡️ Quant System V10.6: Master Advisor & Logic Engine")
    st.markdown("---")
    ...
    def du_bao_xac_suat_ai_t3(bang_du_lieu_da_tinh):
        """
        ...
        if st.button(f"⚡ TIẾN HÀNH PHÂN TÍCH ĐỊNH LƯỢNG MÃ {ma_co_phieu_dang_chon}"):
            ...
            with st.spinner(f"Đang kích hoạt quy trình đồng bộ đa tầng cho mã {ma_co_phieu_dang_chon}..."):
                ...
                if bang_du_lieu_tho is not None and not bang_du_lieu_tho.empty:
                    ...
                    dong_du_lieu_moi_nhat = bang_du_lieu_chi_tiet.iloc[-1]
                    
                    # BƯỚC 3: Gọi các AI và Hàm đo lường lịch sử
                    diem_ai_du_bao_t3 = du_bao_xac_suat_ai_t3(bang_du_lieu_chi_tiet)
                    ...
                    cot_radar_1.metric("Giá Khớp Lệnh Mới Nhất", f"{dong_du_lieu_moi_nhat['close']:,.0f}")
                    ...
                    # RSI
                    chi_so_rsi_trinh_dien = dong_du_lieu_moi_nhat['rsi']
                    ...
                    # MACD
                    chi_so_macd_trinh_dien = dong_du_lieu_moi_nhat['macd']
                    chi_so_signal_trinh_dien = dong_du_lieu_moi_nhat['signal']
                    ...
                    # MAs
                    chi_so_ma20_trinh_dien = dong_du_lieu_moi_nhat['ma20']
                    chi_so_ma50_trinh_dien = dong_du_lieu_moi_nhat['ma50']
                    cot_naked_3.metric("MA20 (Ngắn) / MA50 (Trung)", f"{chi_so_ma20_trinh_dien:,.0f}", delta=f"MA50 hiện tại: {chi_so_ma50_trinh_dien:,.0f}")
                    
                    # BOL
                    chi_so_upper_trinh_dien = dong_du_lieu_moi_nhat['upper_band']
                    chi_so_lower_trinh_dien = dong_du_lieu_moi_nhat['lower_band']
                    ...
                    with st.expander("📖 CẨM NĂNG THỰC CHIẾN GIAO DỊCH (ĐỌC KỸ TRƯỚC KHI XUỐNG TIỀN)"):
                        ...
                        st.write(f"- Sức mạnh Volume ngày hôm nay bằng **{dong_du_lieu_moi_nhat['vol_strength']:.1f} lần** mức trung bình.")
                        ...
                        gia_tri_cat_lo_toi_thieu = dong_du_lieu_moi_nhat['close'] * 0.93
                    ...
                    truc_x_thoi_gian = du_lieu_120_phien_ve['date']
                ...
                for mot_ma_tru in danh_sach_10_ma_tru_quoc_gia:
                    try:
                        ...
                        if dl_tru_tho is not None:
                            ...
                            phien_cuoi_tru = dl_tru_tinh_toan.iloc[-1]
                            
                            # Xác định rõ ràng
                            gia_dang_tang = phien_cuoi_tru['return_1d'] > 0
                            gia_dang_giam = phien_cuoi_tru['return_1d'] < 0
                            khoi_luong_dang_no = phien_cuoi_tru['vol_strength'] > 1.2
                ...
                try:
                    ...
                    if dong_cuoi_cua_ma_quet['vol_strength'] > 1.3:
                        danh_sach_tuyen_chon_hunter.append({
                            ...
                            'Xác Suất Tăng T+3 (AI)': f"{du_bao_xac_suat_ai_t3(df_du_lieu_quet_tinh_xong)}%"
                        })
                except Exception:
                    pass
...
# ==============================================================================
# HẾT MÃ NGUỒN V10.6 THE FINAL EXECUTION
# ==============================================================================
