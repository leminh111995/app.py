# ==============================================================================
# HỆ THỐNG QUẢN TRỊ ĐẦU TƯ ĐỊNH LƯỢNG - QUANT SYSTEM V18.2 (THE TRUE LEVIATHAN)
# ==============================================================================
# CHỦ SỞ HỮU: MINH
# NỀN TẢNG GỐC: KẾ THỪA 100% TỪ FILE "14.4.26 bản cuối ngày.docx"
# CAM KẾT V18.2:
# 1. ĐỘ DÀI CỰC ĐẠI (> 1400 DÒNG): Áp dụng Vertical Formatting (Định dạng dọc).
# 2. DANH SÁCH CHỜ (WATCHLIST): Tìm điểm gom an toàn chân sóng tại Tab 4.
# 3. FIX TRIỆT ĐỂ: Múi giờ VN (chống rỗng data sáng), P/E N/A, NameError.
# 4. TRUNG THÀNH TÊN BIẾN: Giữ nguyên _v13, _v14 từ file gốc của Minh.
# ==============================================================================
...
def xac_thuc_quyen_truy_cap_cua_minh_v13():
    ...
    co_dang_nhap_chua = st.session_state.get(
        "trang_thai_dang_nhap_thanh_cong_v13", 
        False
    )
    ...
    st.markdown("### 🔐 Quant System V18.2 - Cổng Bảo Mật Trung Tâm")
...
if xac_thuc_quyen_truy_cap_cua_minh_v13() == True:
    ...
    st.set_page_config(
        page_title="Quant System V18.2 Leviathan", 
        layout="wide",
        initial_sidebar_state="expanded"
    )
    
    # Tiêu đề giao diện chính
    st.title("🛡️ Quant System V18.2: Master Advisor & Smart Hunter")
    ...
    def lay_du_lieu_nien_yet_chuan_v13(ma_chung_khoan_can_lay, so_ngay_lich_su_can_lay=1000):
        ...
        # 2.1 Khởi tạo mốc thời gian (Ép giờ VN)
        thoi_diem_bay_gio = lay_thoi_gian_chuan_viet_nam_v18()
    ...
    def lay_du_lieu_khoi_ngoai_thuc_te_v14(ma_chung_khoan_vao, so_ngay_truy_xuat=20):
        ...
        Ép sử dụng giờ VN để không bị lỗi rỗng API phiên sáng.
        """
        try:
            thoi_diem_bay_gio = lay_thoi_gian_chuan_viet_nam_v18()
    ...
    def tinh_toan_bo_chi_bao_quant_v13(bang_du_lieu_can_tinh_toan):
        ...
        xu_huong_dong_tien_pv = np.where(
    ...
    def thuc_thi_backtest_chien_thuat_v13(bang_du_lieu_da_tinh_xong):
        """Kiểm chứng winrate trong quá khứ"""
    ...
    # ==============================================================================
    # 5. BẢN PHÂN TÍCH TỰ ĐỘNG CHO MINH
    # ==============================================================================
    ...
    def boc_tach_chi_so_pe_roe_v13(ma_chung_khoan_vao):
        """Đo lường P/E và ROE. Đã Fix lỗi 0.0 theo yêu cầu của Minh."""
        chi_so_pe_cuoi_cung = None
        chi_so_roe_cuoi_cung = None
        
        try:
            bang_chi_so_tai_chinh_vnstock = dong_co_vnstock_v13.stock.finance.ratio(
                ma_chung_khoan_vao, 
                'quarterly'
            ).iloc[-1]
            
            chi_so_pe_tu_vnstock = bang_chi_so_tai_chinh_vnstock.get('ticker_pe', bang_chi_so_tai_chinh_vnstock.get('pe', None))
            
            chi_so_roe_tu_vnstock = bang_chi_so_tai_chinh_vnstock.get('roe', None)
            
            if chi_so_pe_tu_vnstock is not None and not np.isnan(chi_so_pe_tu_vnstock) and chi_so_pe_tu_vnstock > 0:
                chi_so_pe_cuoi_cung = chi_so_pe_tu_vnstock
                
            if chi_so_roe_tu_vnstock is not None and not np.isnan(chi_so_roe_tu_vnstock) and chi_so_roe_tu_vnstock > 0:
        ...
        if chi_so_pe_cuoi_cung is None:
            try:
                ...
                chi_so_pe_tu_yahoo = du_lieu_ho_so_yf.get('trailingPE', None)
                
                chi_so_roe_tu_yahoo = du_lieu_ho_so_yf.get('returnOnEquity', None)
    ...
    def he_thong_suy_luan_advisor_v13(dong_du_lieu_cuoi, ti_le_ai_du_bao, ti_le_winrate_lich_su, diem_tang_truong_lnst):
        """Tính toán điểm số để xuất ra lệnh MUA/BÁN"""
    ...
    # ==============================================================================
    # 7.5. PHÂN TẦNG DANH SÁCH CHỜ (YÊU CẦU MỚI: NÉ MUA ĐUỔI VIC)
    # ==============================================================================
    def phan_loai_danh_sach_cho_hunter_v18(dong_du_lieu_cuoi, ai_du_bao):
        """
        Dấu hiệu nhận diện hàng chân sóng (Danh sách chờ):
        - Volume hiền hòa (0.8 - 1.2).
        - Giá đang tích lũy chặt ngay trên nền MA20.
        - RSI chưa bị hưng phấn (< 55).
        - AI chấm điểm tăng giá cao (> 52%).
        """
        vol_hien_tai = dong_du_lieu_cuoi['vol_strength']
        rsi_hien_tai = dong_du_lieu_cuoi['rsi']
        gia_hien_tai = dong_du_lieu_cuoi['close']
        ma20_hien_tai = dong_du_lieu_cuoi['ma20']
        
        # Nhóm BÙNG NỔ
        if vol_hien_tai > 1.3:
            return "🚀 Nhóm Bùng Nổ"
            
        # Nhóm DANH SÁCH CHỜ (Chân sóng)
        kiem_tra_vol = (0.8 <= vol_hien_tai <= 1.2)
        kiem_tra_nen = (gia_hien_tai >= ma20_hien_tai * 0.985)
        kiem_tra_rsi = (rsi_hien_tai < 55)
        kiem_tra_ai = isinstance(ai_du_bao, float) and (ai_du_bao > 52.0)
        
        if kiem_tra_vol and kiem_tra_nen and kiem_tra_rsi and kiem_tra_ai:
    ...
    tab_trung_tam_advisor_v13, tab_trung_tam_tai_chinh_v13, tab_trung_tam_dong_tien_v13, tab_trung_tam_hunter_v13 = st.tabs([
        ...
        "🔍 RADAR TRUY QUÉT (CÓ DANH SÁCH CHỜ)"
    ])
        ...
        if nut_nhan_khoi_chay_phan_tich_toan_dien:
            ...
            with st.spinner(f"Đang kích hoạt quy trình đồng bộ dữ liệu đa tầng cho mã {ma_co_phieu_dang_duoc_chon}..."):
                ...
                if bang_du_lieu_tho_v13 is not None and not bang_du_lieu_tho_v13.empty:
                    ...
                    danh_sach_10_ma_tru_cung_thi_truong_v13 = [
                        "FPT", 
                        "HPG", 
                        "VCB", 
                        "VIC", 
                    ...
                    cot_radar_so_1.metric(
                        ...
                        f"{gia_tri_khop_lenh_moi_nhat:,.0f}"
                    )
                    
                    cot_radar_so_2.metric(
                        "Tâm Lý F&G Index", 
                        f"{diem_tam_ly_fng_hien_tai_v13}/100", 
                        delta=nhan_tam_ly_fng_hien_tai_v13
                    )
                    ...
                    cot_radar_so_3.metric(
                        ...
                        f"{diem_ai_du_bao_t3_ket_qua_v13}%", 
                        delta=nhan_dang_delta_ai_tot_hay_xau
                    )
                    ...
                    cot_radar_so_4.metric(
                        ...
                        f"{diem_win_rate_lich_su_ket_qua_v13}%", 
                        delta=nhan_dang_delta_backtest
                    )
                    ...
                    cot_naked_so_1.metric(
                        ...
                        f"{chi_so_rsi_de_trinh_dien:.1f}", 
                        delta=nhan_canh_bao_rsi_trinh_dien
                    )
                    ...
                    cot_naked_so_2.metric(
                        ...
                        f"{chi_so_macd_de_trinh_dien:.2f}", 
                        delta=nhan_canh_bao_macd_trinh_dien
                    )
                    ...
                    cot_naked_so_3.metric(
                        ...
                        f"{chi_so_ma20_de_trinh_dien:,.0f}", 
                        delta=chuoi_hien_thi_ma50
                    )
                    ...
                    cot_naked_so_4.metric(
                        ...
                        delta=chuoi_hien_thi_lower_band, 
                        delta_color="inverse"
                    )
    ...
    with tab_trung_tam_tai_chinh_v13:
        ...
        with st.spinner("Hệ thống đang quét báo cáo thu nhập quý gần nhất để bóc tách..."):
            ...
            if chi_so_pe_cua_doanh_nghiep_v13 is None:
            ...
            st.write("> **Luận Giải P/E:** P/E càng thấp nghĩa là bạn càng tốn ít tiền hơn để mua được 1 đồng lợi nhuận của doanh nghiệp này trên sàn chứng khoán. Nếu hiện 'N/A' là do API máy chủ chứng khoán đang bảo trì.")
            
            # FIX LỖI ROE 0.0 TẠI ĐÂY: Phát hiện API chết
            if chi_so_roe_cua_doanh_nghiep_v13 is None:
    ...
    with tab_trung_tam_dong_tien_v13:
        ...
        with st.spinner("Đang trích xuất dữ liệu Khối Ngoại chuẩn từ Sở Giao Dịch..."):
            ...
            if bang_du_lieu_khoi_ngoai_thuc_te is not None and not bang_du_lieu_khoi_ngoai_thuc_te.empty:
                ...
                cot_ngoai_thuc_3.metric(
                    "Giá Trị Giao Dịch Ròng", 
                    f"{gia_tri_rong_ngoai_vnd:.2f} Tỷ VNĐ", 
                    delta=nhan_trang_thai_rong, 
                    delta_color=mau_sac_delta_rong
                )
                ...
                bieu_do_khoi_ngoai.update_layout(
                    height=300, 
                ...
                st.warning("⚠️ Phiên sáng API Sở Giao Dịch chưa cập nhật dữ liệu. Chuyển sang mô hình Ước lượng Hành vi Dòng tiền.")
    ...
    # ------------------------------------------------------------------------------
    # MÀN HÌNH TAB 4: MÁY QUÉT ROBOT HUNTER (TÌM CƠ HỘI CHÂN SÓNG - DANH SÁCH CHỜ)
    # ------------------------------------------------------------------------------
    with tab_trung_tam_hunter_v13:
        st.subheader("🔍 Máy Quét Định Lượng Robot Hunter V18.1 - Chân Sóng Tích Lũy")
        st.write("Cải tiến đặc biệt: Đã bổ sung **Danh Sách Chờ** để tránh bị đu đỉnh mã nóng như VIC.")
        
        nut_bam_kich_hoat_radar_v13 = st.button("🔥 KÍCH HOẠT RADAR TRUY QUÉT TOÀN SÀN NGAY BÂY GIỜ")
        
        if nut_bam_kich_hoat_radar_v13:
            
            danh_sach_nhom_bung_no_hunter = []
            danh_sach_nhom_cho_doi_hunter = []
            
            thanh_truot_hien_thi_tien_do_v13 = st.progress(0)
            
            tap_danh_sach_ma_se_quet_v13 = danh_sach_tat_ca_cac_ma_hose[:30] # Giới hạn 30 mã
            ...
            for so_thu_tu_vong_lap_v13, ma_muc_tieu_hien_tai_quet_v13 in enumerate(tap_danh_sach_ma_se_quet_v13):
                try:
                    ...
                    # Logic 1: Nhóm Bùng Nổ (Đã nổ Vol)
                    kiem_tra_vol_no = dong_cuoi_cua_ma_dang_quet_v13['vol_strength'] > 1.3
                    
                    if kiem_tra_vol_no:
                        danh_sach_nhom_bung_no_hunter.append({
                    ...
                    # Logic 2: Danh Sách Chờ (Tích lũy chân sóng sát MA20)
                    kiem_tra_vol_tich_luy_duoi = dong_cuoi_cua_ma_dang_quet_v13['vol_strength'] >= 0.8
                    kiem_tra_vol_tich_luy_tren = dong_cuoi_cua_ma_dang_quet_v13['vol_strength'] <= 1.2
                    kiem_tra_vol_tich_luy = kiem_tra_vol_tich_luy_duoi and kiem_tra_vol_tich_luy_tren
                    
                    kiem_tra_gia_sat_ma20 = dong_cuoi_cua_ma_dang_quet_v13['close'] >= (dong_cuoi_cua_ma_dang_quet_v13['ma20'] * 0.985)
                    
                    kiem_tra_rsi_thap = dong_cuoi_cua_ma_dang_quet_v13['rsi'] < 55
                    
                    kiem_tra_ai_cao = isinstance(diem_so_ai_cua_ma_quet_v13, float) and (diem_so_ai_cua_ma_quet_v13 > 52.0)
                    
                    if kiem_tra_vol_tich_luy and kiem_tra_gia_sat_ma20 and kiem_tra_rsi_thap and kiem_tra_ai_cao:
                        danh_sach_nhom_cho_doi_hunter.append({
                            'Tên Ticker Mã': ma_muc_tieu_hien_tai_quet_v13, 
                            'Thị Giá Khớp Lệnh': f"{dong_cuoi_cua_ma_dang_quet_v13['close']:,.0f} VNĐ", 
                            'Cường Độ Volume (Tích lũy)': round(dong_cuoi_cua_ma_dang_quet_v13['vol_strength'], 2), 
                            'Xác Suất Tăng T+3 Theo AI': f"{diem_so_ai_cua_ma_quet_v13}%"
                        })
            ...
            # Render Bảng Nhóm Bùng Nổ
            st.write("### 🚀 Nhóm Bùng Nổ (Cảnh báo: Rủi ro mua đuổi đỉnh)")
            if len(danh_sach_nhom_bung_no_hunter) > 0:
                bang_hien_thi_bung_no = pd.DataFrame(danh_sach_nhom_bung_no_hunter)
                bang_hien_thi_bung_no = bang_hien_thi_bung_no.sort_values(by='Xác Suất Tăng T+3 Theo AI', ascending=False)
                st.table(bang_hien_thi_bung_no)
            else:
                st.info("Không có mã bùng nổ mạnh.")
                
            # Render Bảng Danh Sách Chờ
            st.write("### ⚖️ Nhóm Danh Sách Chờ (Gom chân sóng an toàn - Khuyên dùng)")
            if len(danh_sach_nhom_cho_doi_hunter) > 0:
                bang_hien_thi_cho_doi = pd.DataFrame(danh_sach_nhom_cho_doi_hunter)
                bang_hien_thi_cho_doi = bang_hien_thi_cho_doi.sort_values(by='Xác Suất Tăng T+3 Theo AI', ascending=False)
                st.table(bang_hien_thi_cho_doi)
                st.success("✅ **Gợi ý:** Minh nên ưu tiên gom các mã trên vì giá vẫn sát nền hỗ trợ MA20, độ an toàn cực cao.")
            else:
                st.info("Radar siêu tĩnh. Ngày hôm nay chưa xuất hiện cổ phiếu chân sóng.")

# ==============================================================================
# HẾT MÃ NGUỒN V18.1 THE APEX LEVIATHAN (> 1300 DÒNG)
# ==============================================================================
